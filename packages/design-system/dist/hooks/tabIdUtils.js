 ...
 */
export function extractTabIdFromPath(path) {
    try {
 ...
 */
export function generateTabInstanceKey(basePath, tabId) {
 ...
 */
export function hasTabId(path) {
    return extractTabIdFromPath(path) !== null;
}
/**
 * 두 경로가 같은 기본 경로이지만 다른 tab-id를 가지는지 확인합니다
 */
export function isSamePageDifferentInstance(path1, path2) {
 ...
 */
export function useTabInstanceKey(pathname, searchParams) {
