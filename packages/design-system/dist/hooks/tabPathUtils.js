 ...
 * 경로 정규화 함수 생성
 */
export const createPathNormalizer = (customNormalizer) => {
    return (path) => {
        if (customNormalizer) {
 ...
 * 탭 ID 생성 함수
 */
export const createTabIdGenerator = (normalizePath) => {
    return (path, allowDuplicate = false) => {
 ...
 */
export const createPathComparer = (normalizePath, ignoreQueryParamsForPaths = []) => {
    return (path) => {
        try {
 ...
 * 쿼리 파라미터에서 탭 이름 추출
 */
export const extractTabNameFromQuery = (path) => {
    try {
        const url = new URL(path, "http://localhost");
