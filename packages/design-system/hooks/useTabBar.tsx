...
export function useTabBar({
  ...
  const activateOrAddTab = useCallback(
    (pageInfo: PageInfo) => {
      ...
      if (normalizedPageInfo.allowDuplicate) {
        ...
        console.log(
          "activateOrAddTab: Duplicate page - exact path matching only"
        );
      } else {
        // 일반 페이지는 기존 로직 사용
      ...
      if (existingTab) {
        ...
        if (!normalizedPageInfo.allowDuplicate) {
          ...
          console.log(
            "activateOrAddTab: Skipping path update for duplicate page to preserve tab-id"
          );
        }
  ...
  useEffect(() => {
    ...
    if (pageInfo.allowDuplicate && !currentFullPath.includes("tab-id=")) {
      console.log(
        "Direct URL access to duplicate page without tab-id - creating new tab directly"
      );
    ...
    if (!pageInfo.allowDuplicate) {
      ...
      console.log(
        "General tab matching - matchingTab:",
        matchingTab ? matchingTab.id : null
      );
      ...
      if (matchingTab) {
        ...
        if (matchingTab.path !== currentFullPath && !isTabClickNavigation) {
          console.log(
            "Updating tab path for non-duplicate page:",
            currentFullPath
          );
          updateActiveTabTitleAndPath(currentFullPath);
        }
