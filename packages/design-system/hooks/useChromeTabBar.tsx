 ...
 */
export function useChromeTabBar({
  initialTabs = [],
  maxTabs = 10,
  pageMapping = {},
  ...
  localStorageKey = "chrome-tabbar-tabs",
  ...
  faviconResolver,
}: UseChromeTabBarOptions = {}): UseChromeTabBarReturn {
  ...
  const getInitialState = useCallback(() => {
    if (enableLocalStorage) {
      ...
      if (saved && saved.tabs.length > 0) {
        ...
        const restoredTabs: ChromeTabItem[] = saved.tabs.map((tab) => {
          return {
            id: tab.id,
            title: tab.title,
            path: tab.path,
    ...
    const chromeTabItems: ChromeTabItem[] = initialTabs.map((tab) => ({
      id: tab.id,
      title: tab.title,
      path: tab.path,
      favicon: faviconResolver?.(tab.path) || undefined,
  ...
  // 탭 상태
  const [tabs, setTabs] = useState<ChromeTabItem[]>(initialState.tabs);
  ...
  const generateUniqueTabId = useCallback(
  ...
  // 고유한 탭 ID를 포함한 경로 생성
  const generatePathWithTabId = useCallback(
    (basePath: string, allowDuplicate: boolean = false): string => {
  ...
  // 중복 탭 찾기 (쿼리파라미터 고려)
  const findExistingTab = useCallback(
    (
      path: string,
      allowDuplicate: boolean = false
    ): ChromeTabItem | undefined => {
      if (allowDuplicatesByQuery || allowDuplicate) {
  ...
  const addTab = useCallback(
    (pageInfo: PageInfo) => {
      ...
      if (allowDuplicate) {
        ...
        const pathWithTabId = generatePathWithTabId(
          normalizedBasePath,
          allowDuplicate
        );
  ...
  const closeOtherTabs = useCallback(
    (keepTabId: string) => {
      setTabs((prevTabs) => {
        ...
        const homeTabs = prevTabs.filter(
          (tab) => !tab.path.includes("closable=true")
        );

        const newTabs: ChromeTabItem[] = [...homeTabs];
  ...
  const updateActiveTabPath = useCallback(
    (newPath: string) => {
      ...
      if (enableLocalStorage) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const updatedTabs = tabs.map(({ active: _, ...tab }) =>
